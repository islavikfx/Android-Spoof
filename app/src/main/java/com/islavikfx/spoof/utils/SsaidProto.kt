...
object SsidProto {
    ...
    fun parseFile(filePath: String): List<PackageSsid> {
        ...
        try {
            val xmlContent = readFileContent(filePath)
            if (xmlContent.isNotEmpty()) {
                if (isAbxFile(filePath)) {
                    val decodedXml = decodeAbxToXml(filePath)
                    if (decodedXml.isNotEmpty()) {
                        parseXmlContent(decodedXml, result)
                    }
                } else {
    ...
    private fun isAbxFile(filePath: String): Boolean {
        return try {
            val result = Shell.cmd("head -c 3 $filePath 2>/dev/null").exec()
            result.isSuccess && result.out.firstOrNull()?.startsWith("ABX") == true
        } catch (_: Exception) {
            false
        }
    }
    ...
    private fun parseXmlContent(content: String, result: MutableList<PackageSsid>) {
        try {
            val settingPattern = Regex("""<setting\s+[^>]*package="([^"]+)"[^>]*value="([^"]+)"[^>]*(?:defaultValue="([^"]*)")?[^>]*(?:defaultSysSet="([^"]*)")?[^>]*>""")

            for (match in settingPattern.findAll(content)) {
                val groups = match.groupValues
                val packageName = groups[1]
                val value = groups[2]
                val defaultValue = groups.getOrNull(3) ?: value
    ...
    private fun tryParseBinaryRaw(filePath: String, result: MutableList<PackageSsid>) {
        try {
            ...
            if (userKeyIndex != -1) {
                ...
                if (match != null) {
                    result.add(
                        PackageSsid(packageName = "android", ssid = match.value, defaultValue = match.value, defaultSysSet = true)
                    )
                }
            }
            ...
            for (hexMatch in hexPattern.findAll(content)) {
                ...
                if (pkgMatch != null) {
                    ...
                    if (pkgName !in foundPackages && pkgName.contains(".") && !pkgName.endsWith(".")) {
                        ...
                        result.add(
                            PackageSsid(packageName = pkgName, ssid = hexId, defaultValue = hexId, defaultSysSet = false)
                        )
                    }
                }
            }
    ...
    fun updateFile(filePath: String, packageName: String, newValue: String): Boolean {
        try {
            if (updateViaAbx(filePath, packageName, newValue)) {
                return true
            }
    ...
    private fun updateViaAbx(filePath: String, packageName: String, newValue: String): Boolean {
        try {
            ...
            if (isAbxFile(filePath)) {
                ...
                val xmlResult = Shell.cmd("cp $filePath /data/local/tmp/ssid_work.xml 2>/dev/null && cat /data/local/tmp/ssid_work.xml").exec()
                if (!xmlResult.isSuccess || xmlResult.out.isEmpty()) {
                    return false
                }
            }
            ...
            if (isAbxFile(filePath)) {
                ...
                Shell.cmd("cp /data/local/tmp/ssid_work.xml $filePath").exec()
            }
